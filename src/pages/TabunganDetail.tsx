...
const TabunganDetail: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center">
        ...
        <p className="text-gray-400 mt-3 text-xs font-medium">Memuat rincian tabungan...</p>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col pb-8">
        <div className="bg-[#064e3b] px-5 py-4 flex items-center gap-3">
          <button onClick={() => navigate('/tabungan')} className="p-1.5 rounded-full hover:bg-white/10 transition-colors">
            <IconArrowLeft size={20} className="text-white" />
          </button>
          <h1 className="text-base font-bold text-white tracking-wide">Detail Tabungan</h1>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
          ...
          <p className="text-xs text-gray-500 max-w-xs leading-relaxed">
            Data rekening tabungan ini gagal diambil.
          </p>
          <button
  ...
  const { tabungan, mutasi } = responseData.data
    ...
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      >
        ...
        <div className="relative z-10 flex items-start gap-3 mb-6">
          <button onClick={() => navigate('/tabungan')} className="p-1.5 rounded-full hover:bg-white/10 transition-colors mt-0.5">
            <IconArrowLeft size={20} className="text-white" />
          </button>
          <div>
            <h1 className="text-base font-bold text-white tracking-wide">Detail Tabungan</h1>
            <p className="text-[10px] text-[#a7f3d0] mt-1 leading-normal font-medium max-w-[240px]">
              Pantau rincian saldo dan riwayat mutasi kredit/debet secara lengkap.
            </p>
          </div>
        </div>
      </div>

      {/* Main credit card (Blue) — overlaps green */}
        ...
        >
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              ...
              <p className="text-[8px] text-white/40 mt-0.5 uppercase tracking-wider">Tabungan Karyawan</p>
            </div>
          ...
          <div className="relative z-10 my-1 text-center">
            <p className="text-[8px] text-white/50 uppercase tracking-widest">Saldo Rekening</p>
          ...
          <div className="flex justify-between items-end relative z-10">
            <div>
              <p className="text-[8px] text-white/40 uppercase tracking-wider">{tabungan.no_rekening}</p>
              <p className="text-xs font-mono font-bold text-white tracking-[2px]">{tabungan.jenis_tabungan}</p>
            </div>
      ...
      <div className="px-5 mt-6">
        <div className="bg-white rounded-2xl border border-gray-200/60 p-5 shadow-sm flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center border-b border-gray-100 pb-3">
            <div>
              <p className="text-[10px] text-gray-400 font-mono tracking-wider font-semibold uppercase">Informasi Akun</p>
              <h2 className="text-sm font-bold text-gray-800 mt-0.5">{tabungan.jenis_tabungan}</h2>
            </div>
            <span 
              className="text-[8px] px-3 py-1 rounded-md font-extrabold uppercase tracking-wider shadow-sm text-center bg-emerald-500 text-white"
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-xs pb-2.5 border-b border-gray-50">
              ...
              <span className="font-bold text-gray-800 font-mono">{tabungan.no_rekening}</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs pb-2.5 border-b border-gray-50">
              ...
              <span className="font-bold text-gray-800 font-mono">{tabungan.kode_tabungan}</span>
            </div>
            ...
            <div className="flex justify-between items-center text-xs pb-2.5 border-b border-gray-50">
              ...
              <span className="font-bold text-gray-800">{formatDate(tabungan.created_at)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 mt-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200/60 p-4 shadow-sm flex flex-col divide-y divide-gray-50">
          {mutasi.map((item, idx) => {
            ...
            return (
              <div key={item.no_transaksi || idx} className="flex items-center justify-between py-3.5 px-1 first:pt-1 last:pb-1">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-gray-400 mt-0.5 font-medium">
                      {formatDate(item.tanggal)} {item.berita ? `• ${item.berita}` : ''}
                    </p>
                  </div>
                </div>
                <div className="text-right">
